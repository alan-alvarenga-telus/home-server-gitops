...
- `apps/authentik/` — SSO/identity provider, Helm chart from `charts.goauthentik.io`, namespace `authentik`. Multi-source Application (chart + values ref + supplementary kustomize overlay containing the SealedSecret, a bundled redis Deployment/Service, and an explicit Namespace). Exposed via Traefik at `authentik.internal`.
...
```
All `*.internal` traffic terminates here and Traefik routes by Host header.

**Hostname convention:** `<app>.internal` for any app that needs browser access. The cluster expects clients to resolve these names to `192.168.31.218` — set up however you prefer (LAN DNS server with a wildcard, per-workstation `/etc/hosts` entry, etc.). DNS provisioning is out of scope for this repo; the cluster only owns the Ingress configuration.

**Why `.internal` and not `.local`:** `.local` is reserved by mDNS (RFC 6762), which breaks unicast DNS resolution on Apple devices unless Avahi is in the loop. `.internal` is ICANN-reserved for private use (2024) and is on the Public Suffix List as a private TLD — so cookies set on `authentik.internal` don't leak to `kroki.internal`, browsers treat each name as an independent site, and the namespace is guaranteed never to clash with a public TLD. Resolution still comes from your LAN's DNS server (Pi-hole here) via a wildcard `address=/internal/<traefik-ip>` rule; public resolvers return NXDOMAIN for `.internal`, which is exactly what you want.

**To expose a new HTTP app:**
1. In the app's chart values (or `Ingress` manifest), set `ingressClassName: traefik` and `hosts: [<app>.internal]`.
2. Browse to `http://<app>.internal` — Pi-hole resolves it, Traefik routes by Host header.

No need to touch the AppProject — `Ingress` is namespaced, allowed by `apps`' `namespaceResourceWhitelist: '*'`.

**To expose a TCP service (e.g. Postgres):** Traefik doesn't handle non-HTTP by default. Use a `Service` of `type: LoadBalancer` — k3s's klipper-lb assigns it the same node IP as Traefik, but on whatever port you declare. Example: `apps/postgres/overlays/home-server/postgres-rw-external.yaml` exposes the CNPG primary on `5432`. Pi-hole's wildcard means `postgres.internal:5432` resolves to the right IP automatically — no separate DNS record needed.
...
`infrastructure/registry/` runs `registry:2` exposed at `https://registry.internal` via Traefik. htpasswd auth, sealed creds in `infrastructure/registry/overlays/home-server/registry-auth.sealedsecret.yaml`. TLS terminated by a self-signed cert (10-year validity, CN matches hostname) stored in `registry-tls.sealedsecret.yaml`; private key never leaves the cluster.

**Two pieces of host-level state live outside this repo** (one-time setup, documented in `README.md`):
- Workstation: the registry's CA cert at `/etc/docker/certs.d/registry.internal/ca.crt`. Extract from cluster: `kubectl get secret registry-tls -n registry -o jsonpath='{.data.tls\.crt}' | base64 -d`. Docker auto-discovers it; no daemon restart needed.
...
**Build → push → deploy workflow:** `docker build -t registry.internal/<app>:<tag>` → `docker push registry.internal/<app>:<tag>` → reference `image: registry.internal/<app>:<tag>` in any Deployment manifest under `apps/`. The cluster pulls via the containerd config.
